...
> ⚠️ The `/ingest`, `/status`, and `/products` endpoints return `501 Not Implemented`. The CDSE ingestion logic is currently implemented in the service tools (`services/data_ingestor/tools/download_scenes.py`) and will be integrated into the API endpoints in a future release.

## Core Functions (available in service tools)

Business logic is available in `services/data_ingestor/tools/download_scenes.py`:
