...
services/sentinel_preprocessor/tools/sar_preprocessing.py produces IDENTICAL results
to the original implementation in services/sentinel_preprocessor/sar_preprocessing_module.py
on the same input data.

This test meets the requirement:
    ...
    tools implementation -- both implementations must produce
...
from services.sentinel_preprocessor.tools.sar_preprocessing import (
    GCPGeoreferencer as toolsGCPGeoreferencer,
)
from services.sentinel_preprocessor.tools.sar_preprocessing import (
    GCPOutOfBoundsError as toolsGCPOutOfBoundsError,
)
...
# ---------------------------------------------------------------------------
# Test 1 — tools self-consistency (always runs)
# ---------------------------------------------------------------------------


def test_gcp_tools_self_consistency() -> None:
    """tools implementation self-consistency checks.

    These assertions always run and validate that the standalone
    GCPGeoreferencer produces correct results independently.
    """
    tools_gcp = toolsGCPGeoreferencer(_gcps, IMAGE_SHAPE)

    # 1a — Interpolated points are within the expected geographic range
    for line, pixel in CONTROL_POINTS:
        lat_p0, lon_p0 = tools_gcp.pixel_to_latlon(line, pixel)
        assert 29.0 <= lat_p0 <= 31.0, f"tools lat {lat_p0} outside range"
        assert -11.0 <= lon_p0 <= -9.0, f"tools lon {lon_p0} outside range"
    ...
    for i in range(N_LINES):
        for j in range(N_PIXELS):
            ...
            lat_p0, lon_p0 = tools_gcp.pixel_to_latlon(line_gcp, pixel_gcp)
            expected_lat = _gcps[i, j, 0]
            expected_lon = _gcps[i, j, 1]
            assert abs(lat_p0 - expected_lat) < 1e-10, (
                f"tools error at GCP ({i},{j}): {lat_p0} != {expected_lat}"
            )
    ...
    for line, pixel in OUT_OF_BOUNDS:
        with pytest.raises(toolsGCPOutOfBoundsError):
            tools_gcp.pixel_to_latlon(line, pixel)

    # 1d — tile_to_bbox() returns a valid bbox
    bbox_p0 = tools_gcp.tile_to_bbox(25, 25, 75, 75)
    ...
    for i in range(N_LINES):
        for j in range(N_PIXELS):
            ...
            assert lat_p0 == lat_svc, (
                f"GCP ({i},{j}) lat mismatch: tools {lat_p0} != service {lat_svc}"
            )
            assert lon_p0 == lon_svc, (
                f"GCP ({i},{j}) lon mismatch: tools {lon_p0} != service {lon_svc}"
            )
