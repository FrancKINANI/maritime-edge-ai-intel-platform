...
class CalibrationLUT:
    ...
    def __init__(self, calibration_xml_path: str, noise_xml_path: str | None = None):
        ...
        logger.info(
            f"Calibration LUT loaded: sigma shape {self.sigma_values.shape}, "
...
def _apply_pipeline_to_window(
    ...
    noise_lut_window: np.ndarray | None = None,
) -> np.ndarray:
    """
...
def main() -> None:
    ...
    result = process_safe_windowed(
        args.safe, args.pipeline, args.output_dir, args.polarization, args.tile_size, args.overlap
    )
